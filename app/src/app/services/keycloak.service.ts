...
export class KeycloakService {
     ...
     */
    public shouldAddToken?: (request: HttpRequest<unknown>) => boolean;
    /**
     * Returns true if the request being made should potentially update the token.
     */
    public shouldUpdateToken?: (request: HttpRequest<unknown>) => boolean;
    ...
    public async updateToken(minValidity = this._updateMinValidity) {
        ...
        try {
            return await this._instance.updateToken(minValidity as number);
        } catch (error) {
            return false;
        }
    }
    ...
    public addTokenToHeader(headers: HttpHeaders = new HttpHeaders()) {
        return from(this.getToken()).pipe(
            map((token) =>
                token
                    ? headers.set(
                        this._authorizationHeaderName as string,
